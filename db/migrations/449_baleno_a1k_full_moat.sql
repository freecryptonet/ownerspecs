-- 449: Moat fill for Suzuki Baleno (WB / HaynesPro A1K) gen 322. Had fluids(8), battery(2), fuses(62).
-- Missing: torque, tyres, bulbs, svc, parts. 1.2 Dualjet K12C (shares plug/filter with Ignis/Swift).
-- Wheel nuts 100 Nm (HaynesPro-verified, unlike the Swift/Ignis 85). Cite Baleno OM.

SET @gen = 322;   -- baleno-wb-hatchback-2015-2022
SET @eng = (SELECT engine_id FROM fluid_specs WHERE generation_id=@gen AND fluid_type='engine_oil' LIMIT 1);
SET @src = (SELECT id FROM sources WHERE citation LIKE '%Baleno%Owner%' LIMIT 1);
